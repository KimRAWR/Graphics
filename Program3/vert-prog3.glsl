...
void main()
{
    ...
    if (colorByNormals > 0.0) {
      ...
      vec3 Id = .9 * max(0.0, dot(normalize(updated_aNor), L)) * UdColor;

      vec3 H = normalize(L - normalize(vec3(0.0, 0.0, cameraTrans) - (M * V * aPos).xyz));
      float NdotH = max(0.0, dot(normalize(updated_aNor), H));
